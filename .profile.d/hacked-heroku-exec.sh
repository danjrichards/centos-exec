    ...
    if [ $? -ne 0 ]; then
      ...
      (
        ...
        while true; do
          ...
          tunnel=$(curl -s -X POST -d @/tmp/.heroku_exec_data.json -H "Content-Type: application/json" -L $HEROKU_EXEC_URL)
          heroku_exec_log_debug "DanR tunnel=$tunnel"

          echo "$tunnel" | python -c 'import json,sys;obj=json.load(sys.stdin)' > /dev/null 2>&1
          if [ $? != 0 ]; then
            heroku_exec_log_debug "error at=create_tunnel url=$HEROKU_EXEC_URL json=$tunnel"
          else
